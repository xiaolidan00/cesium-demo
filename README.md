# Cesium+ts Demo

## 掘金博客：敲敲敲敲暴你脑袋

- [详细实现过程讲解请看博客](https://juejin.cn/user/224781403162798/posts)

## 绘制折线和多边形

- [源码地址:draw](src/draw/index.ts)

## 自定义 Primitive

- [源码地址:customPrimitive](src/customPrimitive/index.ts)

## 自定义 Primitive 绘制折线和多边形

- [源码地址:drawPrimitive](src/drawPrimitive/index.ts)

## 三角测量

- [源码地址:triangleTool](src/triangleTool/index.ts)

## 通视分析

- [源码地址:viewPoint](src/viewPoint/index.ts)

## 缓冲区分析

- [源码地址:buffer](src/buffer/index.ts)

## 地形开挖

- [源码地址:terrainClip](src/terrainClip/index.ts)

## 等高线

- [源码地址:elevation](src/elevation/index.ts)

## 坡向坡度分析

- [源码地址:terrainOrient](src/terrainOrient/index.ts)

## 填挖方量计算

- [源码地址:terrainFill](src/terrainFill/index.ts)

## 距离面积计算

- [源码地址:distanceArea](src/distanceArea/index.ts)

## 分屏

- [源码地址:split](src/split/index.ts)

## 选择轮廓

- [源码地址:selectOutline](src/selectOutline/index.ts)

## 2D3D 切换

- [源码地址:2D3D](src/2D3D/index.ts)

## HTML 信息框

- [源码地址:htmlOverlay](src/htmlOverlay/index.ts)

## three+cesium

- [源码地址:three](src/three/index.ts)

## Geojosn 行政区块

- [源码地址:geo](src/geo/index.ts)

## 升起柱体行政区块

- [源码地址:bars](src/bars/index.ts)

## 点聚类

- [源码地址:cluster](src/cluster/index.ts)

## 热力平面/山丘

- [源码地址:heatmap](src/heatmap/index.ts)
